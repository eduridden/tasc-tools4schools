...
import type { ToolCategory, TaxonomyItem } from '@/lib/types';
...
export function CategoryManager({ categories }: CategoryManagerProps) {
  ...
  const handleDeletePrompt = (category: ToolCategory) => {
  ...
  const confirmDelete = async () => {
    ...
    deleteDocumentNonBlocking(doc(firestore, 'tool_categories', itemToDelete.id));
    toast({ title: "Category deleted", description: `"${itemToDelete.name}" has been removed.` });
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto rounded-xl border border-base-300">
        <table className="table table-zebra">
          ...
          <tbody>
            {categories.length === 0 && (
              <tr><td colSpan={5} className="text-center text-base-content/40 py-8">No categories yet</td></tr>
            )}
            {categories.map((category) => {
              const Icon = getIcon(category.icon);
              return (
                <tr key={category.id} className="hover:bg-base-200/50">
                  <td>
                    <div
                  ...
                  <td className="text-base-content/60 text-sm max-w-xs truncate">{category.description}</td>
                  <td>
                    <div className="flex items-center gap-2">
                      <div className="h-5 w-5 rounded-full border border-base-300 shadow-sm" style={{ backgroundColor: category.color }} />
                      <code className="text-xs text-base-content/40 font-mono">{category.color}</code>
                    </div>
                  </td>
      ...
      <dialog ref={confirmDialogRef} className="modal modal-middle">
        <div className="modal-box">
          <h3 className="font-bold text-lg text-base-content">Delete Category?</h3>
          <p className="py-4 text-base-content/70">
            This will permanently delete <span className="font-semibold text-base-content">"{itemToDelete?.name}"</span>.
            Any tools in this category will need to be updated.
          </p>
